...
test("workspace preview leads to guide and live service status", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page
    .getByRole("link", { name: "Assessment guide", exact: true })
    .first()
    .click();
  await expect(page).toHaveURL(/\/guide$/);
  await expect(page.getByRole("heading", { level: 1 })).toBeVisible();
  await page
    .getByRole("link", { name: "Workspace status", exact: true })
    .first()
    .click();
