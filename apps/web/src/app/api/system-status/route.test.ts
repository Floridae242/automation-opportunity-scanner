...
describe("system status proxy", () => {
  it("maps valid health responses without exposing service configuration", async () => {
    ...
    const mock = vi
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ status: "ready", database: "private" }),
      });
    ...
    expect(mock.mock.calls[0][0]).toBe(
      "http://internal.example:8000/health/live",
    );
  ...
  it.each([
    [
      "invalid response",
      { ok: true, json: async () => ({ status: "unexpected" }) },
    ],
  ...
  it("handles unreachable services without leaking errors", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockRejectedValue(new Error("postgresql://secret")),
    );
    expect(await (await GET()).json()).toEqual({
      live: "unavailable",
      ready: "unavailable",
