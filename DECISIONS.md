...
## ADR-014 — Provider selection and in-process extraction runs for M3
**Decision:** Extraction providers are config-selected (`AI_PROVIDER=demo|openai_compatible`). `demo` is a deterministic offline sentence-splitter allowed only in development/test — production with `demo` fails closed with 503 `AI_UNAVAILABLE` (no silent fake AI). `openai_compatible` posts to a configurable chat-completions endpoint with JSON mode, bounded retries (max 3, transient/429/parse only), and 30s timeout. Runs execute in-process via FastAPI BackgroundTasks: the row is written `queued`, then `running`/`completed`/`failed` with `error_code`; the intake is never destroyed on failure (USER_FLOWS). Idempotency via optional `Idempotency-Key` header scoped per organization (NFR-R02). Every persisted run records provider, model_id, prompt_version `aos-process-extract v1.0.0`, and schema_version; output passes JSON Schema + Pydantic + semantic validators (unique step keys, evidence refs resolvable to `intake`/`metrics:*`) before any draft version exists (NFR-R01), and drafts created by AI can never carry `reviewed`.  
**Why:** A job queue/Redis is deferred (M0 local-dev doc precedent); one-process workers satisfy the 18-day demo. Fail-closed config keeps AI-security rules (untrusted content, no invented facts) mechanical rather than aspirational.  
**Revisit when:** Multi-instance deployment or >10s typical latency requires the documented async worker (NFR performance).

## ADR-015 — Deterministic dimension anchors and opportunity-run semantics
**Decision:** The M4 engine derives the seven aos-score-v1 dimensions from reviewed facts with documented rule anchors only (frequency bands, manual ratio, rate scaling, integration-status maps, sensitivity/approval risk additions); no LLM participates. `POST /processes/{id}/analyses` against a reviewed version runs the deterministic opportunity engine (task=`opportunity_analysis`, status `completed` inline because it is pure computation) and persists pain points, one candidate opportunity, and an immutable score snapshot; against a draft it performs AI extraction. Re-analysis creates a new run (BR-010). System integration evidence is reviewer-editable on drafts (`PATCH process-versions/{id}.systems`) — the only path by which `integration_ease` becomes computable, enforcing BR-009 (no API claims without evidence). When any dimension lacks reviewed evidence the snapshot yields `provisional` or `insufficient_evidence`, never a fabricated number (BR-001/002, ADR-011). The demo extraction provider parses only the narrative intake, never the metrics line.  
**Why:** Scores must be reproducible and explainable (ADR-003); golden eval vectors match the engine exactly; keeping opportunity analysis deterministic removes AI variance from the highest-stakes output.  
**Revisit when:** Real model providers power pain-point discovery (M6+) — extraction stays AI, scoring stays rules.
